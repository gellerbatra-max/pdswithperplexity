...
| DXF writer | **Real, for piece boundaries and notch positions.** R12 ASCII: HEADER with `$INSUNITS` and `$EXTMIN`/`$EXTMAX`, one BLOCK + POLYLINE per piece, one INSERT each. Arcs written as vertex bulges (exact, round-trip proven); cubics flattened to `FLATTEN_TOLERANCE_MM` and reported. Deterministic — byte-identical for the same document. Every concept it declines to write is reported per piece |
...
The header declares the drawing extents, accumulated *during* the write
rather than computed alongside it — a parallel bounds pass is the version that
goes stale the first time the writer's geometry changes. They cover the
flattened curve, not the vertices: a bulged segment bows outside its own
endpoints, so a vertex-only box would be short by the arc's radius. A document
with no writable geometry omits the variables entirely rather than declaring
an empty box at the origin, which a reader would zoom to.
